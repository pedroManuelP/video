...
int main(int arcg, char** arcgv){
    ...
    cv::VideoWriter out (path, type, cap.get(cv::CAP_PROP_FPS), frameSize, false); // o último argumento define se o vídeo será em cores ou não
    for(counter=0; cap.read(frame); counter++){
        cv::cvtColor(frame, frame, cv::COLOR_BGR2GRAY); // Converte o frame para escala de cinza
